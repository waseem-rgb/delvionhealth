    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage platform aggregators and their rev share</p>
        </div>
      ...
      {!isLoading && !error && (!data || data.length === 0) && (
        <div className="bg-white border border-slate-200 rounded-xl p-12 text-center">
          ...
          <p className="text-slate-500">No aggregator accounts yet.</p>
        </div>
      )}
      ...
      {data && data.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.map((a) => (
            <div key={a.id} className="bg-white border border-slate-200 rounded-xl p-5 space-y-3">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-slate-900 font-semibold">{a.name}</h3>
                  {a.platformName && <p className="text-slate-500 text-xs mt-0.5">Platform: {a.platformName}</p>}
                  {a.contactPerson && <p className="text-slate-500 text-xs">{a.contactPerson} &middot; {a.city || ""}</p>}
                </div>
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${a.status === "ACTIVE" ? "bg-emerald-900/50 text-emerald-400" : "bg-slate-200 text-slate-500"}`}>
                  {a.status}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-slate-900 font-semibold text-sm flex items-center gap-1"><IndianRupee className="h-3 w-3" />{fmt(Number(a.totalRevenue))}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">MTD Revenue</p>
                  <p className="text-slate-900 font-semibold text-sm flex items-center gap-1"><IndianRupee className="h-3 w-3" />{fmt(Number(a.revenueMTD))}</p>
                </div>
              </div>
              {a.platformCommissionPct != null && (
                <p className="text-slate-500 text-xs border-t border-slate-200 pt-2">
